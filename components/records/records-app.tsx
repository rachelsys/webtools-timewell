...
export default function RecordsApp() {
  ...
  useEffect(() => setRecords(loadFocusRecords()), []);
  ...
  }, [records, range, category, result]);

  return <main className="timer-page records-page">
    <SiteHeader current="records" />
    <section className="records-shell">
      <header className="records-title"><div><p className="timer-kicker"><span />MY RECORDS</p><h1>我的專注紀錄</h1><p>不是績效表，只是回頭看見：時間真的有被好好用過。</p></div><a href="/focus">開始一段專注</a></header>
      <section className="records-panel">
        ...
        </article>)}</div> : <div className="records-empty"><span>◌</span><h2>{records.length ? '沒有符合條件的紀錄' : '還沒有專注紀錄'}</h2><p>{records.length ? '換一組篩選條件看看。' : '完成一段專注後，你可以選擇把它留在這裡。'}</p><a href="/focus">開始專注</a></div>}
      </section>
    </section>
