...
import { APP_STORAGE_KEY, DEFAULT_AUDIO_PREFERENCES, getTrackIdForTimer, loadPersistedApp } from '../lib/timer/timer-storage.ts';
...
test('v2 storage migrates safely when trackByTimerId is absent or contains a removed track', () => {
  installStorage();
  storage.set(APP_STORAGE_KEY, JSON.stringify({ version: 2, timer: createTimerState(timer), recentTimers: [], audio: { selectedTrackId: 'track-02', volume: .5, musicMuted: false, alarmEnabled: true, notificationsEnabled: false, trackByTimerId: { noodles: 'removed-track' } }, lastNotifiedRunId: 0 }));
